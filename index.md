...
----
 ...
 - **こちらも Docker Desktop ではない環境で行います。Windowsの方は Powershell の環境で、WSLの環境下で行うということになります。**
 - nips-2026 のディレクトリの中で以下を実行します。
    ```
    ./get_data_nips-2026.sh
    ```
 ...
 - そうすると、データのダウンロードが始まります。トータルで **約55GB** になります。ポケットWi-Fiなどを使っている方はパケットが枯渇しますので、インターネット環境で作業されてください。また、ダウンロードには 4-5時間かかると思います。様々な環境を考慮し、できるだけダウンロードが失敗しないようにプログラムを書いてあります。ゆっくりお待ちください。そして、ダウンロードされたファイルが展開されると、さらに **約100GB** 使用します。チュートリアルでさらに容量が増えますので、外付けディスクは十分に余裕をとってください。
