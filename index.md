...
### 5-1-2. macOS で Docker Desktop を使用する場合
 - **【ポイント】データのダウンロードは Docker の外で行います。今は Docker Desktop は使用しません。**
...
- `nips-2026tc` フォルダの中に `archive` `s02` `s02_ru` `s02_prebaked` `Group` `Batch_GUI` などのフォルダができます
　
...
----
 ...
 - nips-2026 のディレクトリの中で以下を実行します。
    ...
    git pull
    ```
 - こうすると、```nips-2026tc``` 以下に保存されている配布ファイルに更新があった場合、アップデートされます。随時、ファイルを更新しますので、アナウンスがあれば、こちらのコマンドを実行していただきます。
